...
# Simple in-memory cache for generic jobs fetch
JOBS_CACHE = {
    "data": None,
    "timestamp": 0
}
...
def get_jobs():
    ...
    try:
        ...
        # If no coordinates provided, use cache to speed up generic student dashboard requests
        if user_lat is None and user_lng is None:
            if JOBS_CACHE["data"] is not None and (time.time() - JOBS_CACHE["timestamp"] < CACHE_TTL):
                return jsonify({"data": JOBS_CACHE["data"]}), 200

        # Only show open jobs to students, fetching max 200 to prevent payloads becoming massive
        res = supabase.table('jobs').select('*, profiles!jobs_employer_id_fkey(full_name, avatar_url, company_name)').eq('status', 'open').order('created_at', desc=True).limit(200).execute()
        jobs = res.data or []
        ...
        if user_lat is not None and user_lng is not None:
            ...
            # Cap the final payload to prevent frontend map lag
            jobs = jobs[:100]
        else:
            # Default sort by newest if no location
            jobs.sort(key=lambda x: x.get('created_at', ''), reverse=True)
            # Cap payload for generic view
            jobs = jobs[:100]
            
            # Update cache
            JOBS_CACHE["data"] = jobs
            JOBS_CACHE["timestamp"] = time.time()
...
def create_job():
    ...
    try:
        ...
        wage = data.get("wage")
        if not title:
            return jsonify({"error": "Validation", "message": "Job title is required."}), 400
        if not location:
            return jsonify({"error": "Validation", "message": "Workplace location is required."}), 400
        if wage is None or str(wage).strip() == '':
            return jsonify({"error": "Validation", "message": "Wage is required."}), 400
...
def update_job(job_id):
    ...
    try:
        ...
        allowed_fields = ['title', 'description', 'location', 'wage', 'slots', 'requirements', 'urgent', 'status', 'image_url']
...
def delete_job(job_id):
    ...
    try:
        ...
        # otherwise we manually delete relational data first to prevent Orphaned constraints or FK violation crashes
        ...
        supabase.table('ratings').delete().eq('job_id', job_id).execute()
...
# Deprecated /api/reviews endpoints removed to prevent rate-bombing vulnerabilities.
# The robust 2-way rating system now operates entirely on /api/ratings with constraint checks.
...
def apply_job():
    ...
    try:
        ...
        existing_app = supabase.table('applications').select('id').eq('job_id', job_id).eq('student_id', student_id).execute()
        if existing_app.data:
            return jsonify({"error": "Already applied", "message": "You have already applied for this job."}), 409

        # Verify the job is still open
        job_check = supabase.table('jobs').select('status').eq('id', job_id).single().execute()
        if not job_check.data or job_check.data.get('status') != 'open':
            return jsonify({"error": "Job unavailable", "message": "This job is no longer accepting applications."}), 409
...
def update_application_status():
    ...
    try:
        ...
        # --- FIX: whitelist allowed status values ---
        ALLOWED_STATUSES = {'accepted', 'rejected', 'pending'}
        if new_status not in ALLOWED_STATUSES:
            return jsonify({"error": "Invalid status", "message": f"Status must be one of: {', '.join(ALLOWED_STATUSES)}"}), 400
        ...
        # --- Auto-close job when all slots are filled ---
        if new_status == 'accepted':
            try:
                job_res = supabase.table('jobs').select('slots, status').eq('id', job_id).single().execute()
                job_slots = (job_res.data or {}).get('slots', 1) or 1
                job_status = (job_res.data or {}).get('status', 'open')

                if job_status == 'open':
                    accepted_res = supabase.table('applications').select('id', count='exact').eq('job_id', job_id).eq('status', 'accepted').execute()
                    accepted_count = accepted_res.count or 0

                    if accepted_count >= job_slots:
                        supabase.table('jobs').update({'status': 'closed'}).eq('id', job_id).execute()

                        pending_res = supabase.table('applications').select('student_id').eq('job_id', job_id).eq('status', 'pending').execute()
                        for pending_app in (pending_res.data or []):
                            create_notification(
                                pending_app['student_id'],
                                f"All slots for '{job_title}' have been filled. The position is now closed.",
                                "job_closed"
                            )
            except Exception as slot_err:
                print(f"[SLOTS] Error during slot-fill check: {slot_err}")
...
@require_auth
def submit_rating():
    """Submit a rating for another user (employer rating student, or student rating employer)"""
    try:
        data = request.json
        ...
        if not target_id or not job_id or not rating:
            return jsonify({"error": "Missing required fields: target_id, job_id, rating"}), 400
        ...
        # Prevent duplicate rating for the same job and target
        existing = supabase.table('ratings').select('id').eq('reviewer_id', reviewer_id).eq('target_id', target_id).eq('job_id', job_id).execute()
        if existing.data:
            return jsonify({"error": "Duplicate", "message": "You have already rated this user for this job."}), 409
        ...
        create_notification(target_id, f"You received a new {rating_val}-star rating from your recent job!", "rating")
        
        return jsonify({"data": res.data[0] if res.data else {}, "message": "Rating submitted successfully"}), 201
...
def get_user_ratings(user_id):
    """Get all ratings and stats for a specific user"""
    try:
        # Fetch the ratings where target_id == user_id
        try:
            res = supabase.table('ratings').select('*, profiles!ratings_reviewer_id_fkey(full_name, role, pfp_url), jobs!ratings_job_id_fkey(title)').eq('target_id', user_id).order('created_at', desc=True).execute()
        except:
            res = supabase.table('ratings').select('*').eq('target_id', user_id).order('created_at', desc=True).execute()
        
        ratings_list = res.data or []
        count = len(ratings_list)
        average = sum(r['rating'] for r in ratings_list) / count if count > 0 else 0
...
def submit_report():
    ...
    try:
        ...
        # --- FIX: require reason AND at least one target ---
        if not reason:
            return jsonify({"error": "Missing reason", "message": "A reason is required to submit a report."}), 400
        if not reported_id and not job_id:
            return jsonify({"error": "Missing target", "message": "You must report either a user or a job."}), 400
...
def admin_delete_job(job_id):
    ...
    try:
        ...
        # --- FIX: clean up all related records before deleting the job ---
        supabase.table('applications').delete().eq('job_id', job_id).execute()
        supabase.table('shifts').delete().eq('job_id', job_id).execute()
        supabase.table('saved_jobs').delete().eq('job_id', job_id).execute()
...
def toggle_ban_user(user_id):
    ...
    try:
        ...
        # Sub-action: If banned, close employer jobs + reject student pending applications
        if is_banning:
            try:
                # Close all active jobs if employer
                supabase.table('jobs').update({'status': 'closed'}).eq('employer_id', user_id).execute()
            except Exception as e:
                print("Failed to close jobs for banned employer:", e)
            try:
                # Reject all pending applications if student
                supabase.table('applications').update({'status': 'rejected'}).eq('student_id', user_id).eq('status', 'pending').execute()
            except Exception as e:
                print("Failed to reject applications for banned student:", e)
...
def approve_verification():
    ...
    try:
        ...
        # --- FIX: sync is_verified on BOTH approve AND reject ---
        profile_update = {'is_verified': status == 'approved'}
        supabase.table('profiles').update(profile_update).eq('id', employer_id).execute()

        # Notify the employer
        msg = "Your business verification has been approved! You can now post jobs." if status == 'approved' \
              else f"Your verification was rejected. Reason: {notes or 'Please contact support.'}"
        create_notification(employer_id, msg, "verification")
